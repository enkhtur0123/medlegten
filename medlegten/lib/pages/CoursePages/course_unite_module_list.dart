        ...
        child: Column(
          ...
          children: [
                ...
                child: Row(
                  ...
                  children: [
                    Column(
                      ...
                      children: [
                        Text('UNIT '+unitInfo.unitNumber+': '+unitInfo.unitName.toUpperCase(),
                          style: TextStyle(color: colorPrimary, fontFamily: 'Roboto', fontWeight: FontWeight.w700, fontSize: 16),
                        ),
                        addVerticalSpace(8),
                        Row(
                          children: [
                    ...
                    CircularPercentIndicator(
                        ...
                        lineWidth: 3.0,
                        percent: 0.6,
                        center: Text("60%", style: TextStyle(color: colorPrimary),),
              ...
              Padding(
                ...
                child: Divider(color: Color.fromRGBO(199, 201, 217, .2), thickness: 1,),
              ),
              Expanded(
                      ...
                      child: Column(
                        children: [
                          FutureBuilder<List<CourseUnitModuleList>?>(
                              future: LandingRepository().getCourseUnitModuleList(unitInfo.unitId),
                              builder: (BuildContext context,AsyncSnapshot<List<CourseUnitModuleList>?> snapshot) {
                                if (snapshot.hasData) {
                                  List<Tuple2<int, CourseUnitModuleList>> listTuple = <Tuple2<int, CourseUnitModuleList>>[];
                                  for(int i=0; i<snapshot.data!.length; i++) {
                                    listTuple.add(Tuple2<int, CourseUnitModuleList>(i, snapshot.data![i]));
                                  }
                                  return Column(
                                      mainAxisAlignment: MainAxisAlignment.start,
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: listTuple
                                          .map( (tuple) => buildTimeline(tuple.item2, tuple.item1 == 0 ? 0 : tuple.item1 == listTuple.length- 1? 1 : -1, context))
                                          .toList()
                                  );
                                }
        ...
        child: GestureDetector(
          onTap: () {
            switch(data.moduleTypeId) {
              ...
              default: { print('no module'); }
            }
          },
          child: Container(
            child: Text(data.moduleTypeName, style: TextStyle(color: colorPrimary, fontWeight: FontWeight.w700, fontSize: 16),),
          ),
        )
      ),
    ),
